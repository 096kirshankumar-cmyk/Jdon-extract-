...
before the container started, so whether the deploy included it is uncertain.
...
`bbe4221` added the per-method breakdown and the warning precisely because the
log did not say which of the three sub-causes fired. **This run predates it.**
...
OPH-001 q4 was clean in this same run, but OPH-006 q3 bled 284 characters.
The bleed is **model recitation**, which is non-deterministic — consistent with
the earlier evidence that the amount varied between runs (696 → 121 → 0 → 284).

The prompt rule added in `b0b7889` reduces it but does not eliminate it. The
`sanitize_solution_text` backstop is catching every instance, so **no bled
text has ever shipped** — but relying on cleanup is not a fix.
...
Three across the book (OPH-002 p23, OPH-005 p101, OPH-006 p117). These are
figures the book prints that no ownership level claimed. Small count, each a
distinct case.

## Finding 4 — the discard path is working as designed
...
An unusable item was dropped instead of shipped. That is the intended
behaviour, and it is why OPH-007 still ended clean.

## Revised priority

1. **Intermittent key-table failure** — ~110 blockers. Needs one run with
   `bbe4221` so the warning line identifies the sub-cause on a failing
   chapter (OPH-004 or OPH-018).
...
4. **`contaminated_question`** (3 instances) and **option-text fabrication**
   — real extraction defects, unaffected by the above.
